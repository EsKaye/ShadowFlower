...
export class GeminiProvider {
    ...
    async analyzeItem(item) {
        ...
        try {
            const response = await axios.post(`${this.baseUrl}/models/${this.config.model}:generateContent?key=${this.config.apiKey}`, {
                contents: [{
                        parts: [{
                                text: prompt
                            }]
    ...
    async healthCheck() {
        ...
        try {
            const response = await axios.get(`${this.baseUrl}/models/${this.config.model}?key=${this.config.apiKey}`, { timeout: 5000 });
...
`;
    ...
    validateAction(action) {
        const validActions = ['approve', 'review', 'escalate', 'remove', 'suspend'];
        return validActions.includes(action) ? action : 'review';
    }
    validateSeverity(severity) {
        const validSeverities = ['low', 'medium', 'high', 'critical'];
        return validSeverities.includes(severity) ? severity : 'medium';
    }
    createDefaultResponse(_itemId, error) {
        return {
