...
export interface GameDinClientConfig {
  ...
  apiKey: string;
  timeout?: number;
  signingSecret?: string; // Optional HMAC signing secret for privileged requests (SHADOWFLOWER_SIGNING_SECRET)
}
...
export class GameDinClient {
  ...
  async fetchReviewableReports(): Promise<any> {
    try {
      ...
      const status = error instanceof Error && 'response' in error ? (error as any).response?.status : 'unknown';
